...
module('Integration | Component | th sortable', function (hooks) {
  ...
  test('sort is reactive', async function (assert) {
    ...
    await render(
      <template>
        <RawDataTable @content={{content}} @fields="name" @sortableFields="name" @sort={{context.sort}} @updateSort={{fn (mut context.sort)}} />
      </template>
    );
    ...
    await click('.raw-data-table .sortable span');
    assert.equal(context.sort, null, 'sort value got cleared after three clicks');
  });
  ...
  test('fields are sortable by default', async function (assert) {
    ...
    await render(
      <template>
        <RawDataTable @content={{content}} @fields="name" />
      </template>
    );
  ...
  test('disable sorting with empty string or list', async function (assert) {
    ...
    assert.dom('.raw-data-table .sortable span').doesNotExist('no sortable header if empty string');

    const emptyList = [];
    await render(
      <template>
        <RawDataTable @content={{content}} @fields="name" @sortableFields={{emptyList}} />
      </template>
    );

    assert.dom('.raw-data-table .sortable span').doesNotExist('no sortable header if empty array');

    await render(
      <template>
        <RawDataTable @content={{content}} @fields="name" @sortableFields="InexistingField" />
      </template>
    );
