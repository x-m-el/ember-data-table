...
module('Integration | Component | number pagination', function (hooks) {
  ...
  test('it renders', async function (assert) {
    ...
    await render(
      <template>
        <RawDataTable @page={{page}} @meta={{meta}} />
      </template>,
    );
  ...
  test('page and size are reactive', async function (assert) {
    const content = [{ id: 1, name: 'John Doe' }, { id: 2, name: 'Jane Doe' }, { id: 3, name: 'John Smith' }, { id: 4, name: 'Jane Smith' }];
    ...
    await render(
      <template>
        <RawDataTable @fields="name" @content={{content}} @meta={{meta}} @page={{context.page}} @size={{context.size}} @updatePage={{fn (mut context.page)}} @updatePageSize={{fn (mut context.size)}} />
      </template>
    );
  ...
  test('set sizes via @sizes', async function (assert) {
    ...
    await render(
      <template>
        <RawDataTable @fields="name" @content={{content}} @meta={{meta}} @sizes={{sizes}} @size={{currentSize}} />
      </template>
    );

    assert.dom('.data-table-pagination-left select option').exists({ count: sizes.length }, 'does not render extra sizes');
    assert.dom('.data-table-pagination-left select option').hasText('111', 'renders first size');
    assert.dom('.data-table-pagination-left select').hasValue('111', 'renders first size');
    assert.dom('.data-table-pagination-left select option:last-child').hasText('222', 'renders second size');
  });
  ...
  test('allow to disable sizes via @sizes', async function (assert) {
    ...
    await render(
      <template>
        <RawDataTable @fields="name" @content={{content}} @meta={{meta}} @sizes={{sizes}} />
      </template>
    );

    assert.dom('.data-table-pagination-left select').doesNotExist('does not render size select if null');

    await render(
      <template>
        <RawDataTable @fields="name" @content={{content}} @meta={{meta}} @sizes="" />
      </template>
    );

    assert.dom('.data-table-pagination-left select').doesNotExist('does not render size select if empty string');
  });
});
