                      ...
                      {{#each header.fields as |field|}}
                        <header.ThSortable @field={{field}} @hasCustomBlock={{has-block "data-header"}} as |dataHeader|>
                          ...
                          {{else if dataHeader.customHeaderComponent}}
                            {{#let dataHeader.customHeaderComponent as |CustomComponent|}}
                          ...
                          {{else}}
                            ...
                            >
                              {{#if row.enableSelection}}
                                <td class="center">
                                  <input type="checkbox" aria-label="Select row" checked={{row.isSelected}} {{on "click" row.toggleSelected}} />
                                </td>
                              {{/if}}
                              ...
                              <row.DataCells as |dataCells|>
                                ...
                                {{else}}
                                  ...
                                  {{#if dataCells.firstColumnField}}
                                    <dataCells.DataCell @column={{dataCells.firstColumnField}} @hasCustomBlock={{has-block "data-cell"}} as |cell|>
      ...
      {{else}}
        <div class="data-table-pagination">
          <div class="data-table-pagination-left">
            Displaying {{pagination.startIndex}}-{{pagination.endIndex}}
          ...
          {{#if pagination.hasMultiplePages}}
            <div class="data-table-pagination-right">
              <button disabled={{pagination.isFirstPage}} type="button" {{on "click" (fn (mut pagination.humanPage) pagination.firstPage)}}>First</button>
              <button disabled={{pagination.isFirstPage}} type="button" {{on "click" (fn (mut pagination.humanPage) pagination.previousPage)}}>Previous</button>

              <button disabled={{pagination.isLastPage}} type="button" {{on "click" (fn (mut pagination.humanPage) pagination.nextPage)}}>Next</button>
              <button disabled={{pagination.isLastPage}} type="button" {{on "click" (fn (mut pagination.humanPage) pagination.lastPage)}}>Last</button>
            </div>
          {{/if}}
        </div>
      {{/if}}
